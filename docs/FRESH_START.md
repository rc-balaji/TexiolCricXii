# v0.3.0 Fresh Start checklist

This version intentionally starts a new account schema. Local SharedPreferences keys are bumped to `v4`, so old v0.2.x CricXii account state is not automatically treated as a valid v0.3 login.
...
3. Delete old Firestore data before first v0.3 test if you want a completely clean database.
4. Deploy this ZIP's `firestore.rules` and `firestore.indexes.json`.
5. Build/install v0.3.0 and register the first CricXii account from the app.
...
## Old collections that v0.3 no longer uses
