...
## v1.6.3 custom-avatar privacy note

A custom avatar's owner-supplied HTTPS source URL is private account data. It is stored only in the owner's local/account state and is not written to the public `players/{playerId}` document or shared match participant snapshots. When the URL is saved, the app downloads the image on the owner's device, validates it, and publishes the original image bytes unchanged across authenticated deterministic Firestore chunks. The avatar image itself is therefore shareable/profile-visible at source quality; the original source URL remains private.

### Custom avatar privacy

Custom avatar source URLs remain in the owner-only account state. Public player and match documents never contain that source URL. CricXii stores the downloaded image bytes unchanged in authenticated, deterministic `players/{playerId}/avatarChunks/*` documents so other signed-in players can render the same image without learning the source location.
