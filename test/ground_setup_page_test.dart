...
class FakeAr extends GroundArGateway {
  ...
  Future<GroundLayout?> open(GroundLayout layout) async {
    opens++;
    if (failOpen) throw PlatformException(code: 'camera_denied', message: 'Camera access was denied.');
    return layout.copyWith(lengthMetres: 15);
  }
}

Future<void> showPage(WidgetTester tester, MemoryRepository repository, FakeAr ar,
    {double width = 390, double textScale = 1}) async {
...
void main() {
  testWidgets('unsupported device keeps full manual editor and never opens AR', (tester) async {
    final ar = FakeAr(GroundArSupport.unsupported);
    await showPage(tester, MemoryRepository(), ar, width: 320);
    expect(find.textContaining('AR unavailable on this device.'), findsOneWidget);
    expect(tester.widget<FilledButton>(find.byKey(const ValueKey('open-ground-ar'))).onPressed, isNull);
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('custom-pitch-length')), '18');
  ...
  testWidgets('invalid custom length stays editable, does not mutate saved layout', (tester) async {
    final repo = MemoryRepository();
    await showPage(tester, repo, FakeAr(GroundArSupport.unsupported));
    await tester.ensureVisible(find.byKey(const ValueKey('custom-length')));
    await tester.tap(find.byKey(const ValueKey('custom-length')));
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('custom-pitch-length')), 'NaN');
    await tester.tap(find.text('Apply'));
    await tester.pump();
    expect(find.text('Enter a distance between 4 and 40 metres in yards.'), findsOneWidget);
  ...
  testWidgets('native returned dimensions save and restore without world placement', (tester) async {
    final repo = MemoryRepository();
  ...
  testWidgets('camera denial recovers button and reports useful failure', (tester) async {
    ...
    expect(tester.widget<FilledButton>(find.byKey(const ValueKey('open-ground-ar'))).onPressed, isNotNull);
    expect(tester.takeException(), isNull);
  });

  testWidgets('large text and narrow phone keep all controls usable', (tester) async {
    await showPage(tester, MemoryRepository(), FakeAr(GroundArSupport.unsupported), width: 320, textScale: 1.6);
  ...
  testWidgets('failed save leaves layout available for retry', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(find.text('Could not save the layout. Please try again.'), findsOneWidget);
    expect(repo.saved, isNull);
    expect(tester.takeException(), isNull);
  });
}
