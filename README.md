# CricXii v1.6.3 — Original-Quality Avatars & Match-Day UX
...
- On save, CricXii keeps the owner-supplied URL private and publishes the downloaded image bytes unchanged in authenticated Firestore chunks, so other players see the same-quality avatar without receiving the source URL.
- Multiple private avatar URLs are saved as named choices with visible previews and an explicit selected state.


## v1.6 highlights

- Team Match scorecards now use a conventional cricket layout: innings score strip, batting with dismissal details, Extras, Total, Yet to Bat, bowling figures, Fall of Wickets and Partnerships.
- Team labels include captain `(c)`, wicketkeeper `(wk)` and Joker `(J)` markers where applicable.
- Singles matches now show a scorecard first and the official CricXii ranking separately below it.
- Singles PDF export contains the scorecard and ranking as separate report sections/pages.
- Today Performance full-match preview/PDF embeds the same full scorecards instead of only compact ranking tables.
- Large Team scorecards and repeated Super Overs remain MultiPage-safe.
...
- Match PDFs use CricXii branding and cricket-style scorecard sections; Singles keeps a separate ranking page and Team innings can paginate for large rosters.
